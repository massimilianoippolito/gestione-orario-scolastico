...
def mostra_vista_assegnazioni():
    ...
    with tab_gestione:
        ...
        st.dataframe(pd.DataFrame(riepilogo_ore), use_container_width=True, hide_index=True)
    ...
    with tab_per_docente:
        ...
        if not assegnazioni_docente:
            ...
            st.dataframe(df_docente_dettaglio, use_container_width=True, hide_index=True)
    ...
    with tab_per_classe:
        ...
        # Calcoliamo le ore totali previste dalla griglia oraria della classe (se configurate)
        ...
        if not assegnazioni_classe:
            ...
            st.dataframe(df_classe_dettaglio, use_container_width=True, hide_index=True)
            
            # Controllo di coerenza monte ore
    ...
    # -------------------------------------------------------------
    # SEZIONE DI EMERGENZA: SVUOTA TUTTE LE ASSEGNAZIONI ( ISOLATA AL PROGETTO )
    # -------------------------------------------------------------
    with st.expander(f"⚠️ Zona Pericolosa: Gestione Assegnazioni ({progetto_corrente})", expanded=False):
        ...
        if st.button(f"🗑️ Rimuovi Tutte le Assegnazioni di '{progetto_corrente}'", type="primary", use_container_width=True):
            if conferma_reset_ass:
                ...
                cursor = conn.cursor()
                try:
                    # Cancella solo le assegnazioni del progetto corrente
                    cursor.execute("DELETE FROM assegnazioni WHERE progetto = ?", (progetto_corrente,))
                    
                    # Se esistono tabelle di risultato orario collegate al progetto, puliamo anche quelle in sicurezza
