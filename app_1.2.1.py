...
with st.sidebar:
    ...
    with st.container(height=450):
        ...
        # --- IMPORTAZIONE PROGETTO DA JSON, TABULONE EXCEL O ORARIO FACILE (XML) ---
        with st.expander("📂 Carica Progetto / Importa"):
           # tipo_import = st.radio("Sorgente importazione", ["JSON nativo", "Tabellone Excel (.xlsx)", "Orario Facile (XML)"])
            tipo_import = st.radio("Sorgente importazione", ["JSON nativo"])
            
            if tipo_import == "JSON nativo":
            ...
            elif tipo_import == "Tabulone Excel (.xlsx)":
                file_excel = st.file_uploader("Seleziona tabulone Excel (es. orario provvisorio)", type=["xlsx", "xls"], key="upl_excel_side")
                if file_excel is not None:
                    if st.button("Elabora e Importa Tabulone", use_container_width=True):
                        try:
                            xls_file = pd.ExcelFile(file_excel)
                            sheet_to_use = "ORARIO" if "ORARIO" in xls_file.sheet_names else xls_file.sheet_names[0]
                            df_excel = pd.read_excel(file_excel, sheet_name=sheet_to_use)
                            ...
                            row_days = df_excel.iloc[2]
                            row_hours = df_excel.iloc[3]
                            
                            mapping_cols = {}
                            current_day = "Lunedì"
                            
                            for col_idx in range(len(df_excel.columns)):
                                d_val = row_days.iloc[col_idx]
                                if pd.notna(d_val) and str(d_val).strip() != "":
                                    raw_d = str(d_val).strip().capitalize()
                                    if "luned" in raw_d.lower(): current_day = "Lunedì"
                                    elif "marted" in raw_d.lower(): current_day = "Martedì"
                                    elif "mercoled" in raw_d.lower(): current_day = "Mercoledì"
                                    elif "gioved" in raw_d.lower(): current_day = "Giovedì"
                                    elif "venerd" in raw_d.lower(): current_day = "Venerdì"
                                    elif "sabato" in raw_d.lower(): current_day = "Sabato"
                                    
                                h_val = row_hours.iloc[col_idx]
                                if pd.notna(h_val) and str(h_val).strip().isdigit():
                                    mapping_cols[col_idx] = (current_day, int(str(h_val).strip()))

                            importate_counter = 0
                            
                            for row_idx in range(4, len(df_excel)):
                                docente_name = df_excel.iloc[row_idx, 3]
                                if pd.isna(docente_name) or str(docente_name).strip() == "":
                                    continue
                                    
                                docente_clean = str(docente_name).strip()
                                
                                for col_idx, (giorno, ora) in mapping_cols.items():
                                    classe_val = df_excel.iloc[row_idx, col_idx]
                                    if pd.notna(classe_val) and str(classe_val).strip() != "":
                                        classe_clean = str(classe_val).strip()
                                        
                                        cursor.execute(
                                            """INSERT INTO orario_risultato (classe, giorno, ora, materia, docente, progetto) 
                                               VALUES (?, ?, ?, ?, ?, ?)""",
                                            (classe_clean, giorno, ora, "Lezione", docente_clean, prog)
                                        )
                                        importate_counter += 1
                            ...
                            if importate_counter > 0:
                                st.success(f"✅ Tabulone importato con successo! Elaborate {importate_counter} collocazioni orarie.")
                                st.rerun()
                            else:
                                st.warning("⚠️ Nessuna ora trovata. Controlla che il file rispetti la struttura del tabulone standard.")
                                
                        except Exception as e:
                            st.error(f"Errore durante l'elaborazione del tabulone Excel: {e}")

            elif tipo_import == "Orario Facile (XML)":
                file_xml = st.file_uploader("Seleziona file XML di Orario Facile", type=["xml"], key="upl_xml_side")
                if file_xml is not None:
                    if st.button("Elabora e Importa XML", use_container_width=True):
                        try:
                            with open("temp_orariofacile.xml", "wb") as f:
                                f.write(file_xml.getbuffer())
                            
                            successo, messaggio = importa_xml_orariofacile("temp_orariofacile.xml", progetto=st.session_state.progetto_corrente)
                            
                            if os.path.exists("temp_orariofacile.xml"):
                                os.remove("temp_orariofacile.xml")
                            ...
                            st.error(f"Errore durante l'importazione XML: {e}")
