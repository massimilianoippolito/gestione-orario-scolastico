...
# --- INIZIALIZZAZIONE DATABASE (UTENTI + TABELLE PROGETTO) ---
def inizializza_db_locale():
    ...
    # Creazione automatica account admin di default se la tabella è vuota
...
with st.sidebar:
    ...
    # --- ESPORTAZIONE (DOWNLOAD JSON) ---
    st.subheader("Salva Progetto")
    nome_file_export = st.text_input("Nome file di salvataggio", value=f"orario_{st.session_state.username}")
    ...
    if file_caricato is not None:
        if st.button("Carica nel mio Account"):
            try:
                ...
                # Pulizia dati attuali dell'utente prima del caricamento
                cursor.execute("DELETE FROM classi WHERE progetto = ?", (prog,))
                cursor.execute("DELETE FROM docenti WHERE progetto = ?", (prog,))
                cursor.execute("DELETE FROM materie WHERE progetto = ?", (prog,))
                cursor.execute("DELETE FROM assegnazioni WHERE progetto = ?", (prog,))
